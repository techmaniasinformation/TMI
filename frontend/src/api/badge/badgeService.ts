...
import type { BadgeListResponse, MemberBadgeResponse, MemberBadge, PatchRepBadgeResponse } from "@/types/mypage/badge";

// 전체 배지 메타 조회
export async function fetchAllBadges() {
  try {
    const data: BadgeListResponse = await getRequest('/badge');
...
export async function fetchMemberBadges(memberId: number): Promise<MemberBadge[]> {
  try {
    const data: MemberBadgeResponse = await getRequest(`/memberBadge?memberId=${memberId}`);
...
export async function patchRepresentativeBadge(memberBadgeId: number): Promise<PatchRepBadgeResponse> {
  try {
    const data: PatchRepBadgeResponse = await patchRequest(`/memberBadge/${memberBadgeId}`, {});
