    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-[512px] h-[220px] px-6 pt-6 relative">
        ...
        <div className='pt-4'>
          ...
          {/* 에러 메시지 */}
          {errorMessage && (
            <p className="text-center text-sm text-red-600 mb-2">{errorMessage}</p>
          )}
          ...
          <div className="flex justify-end space-x-2">
            <button
              onClick={onCancel}
              disabled={loading}
              className={`px-4 py-2 border border-gray-300 rounded-md text-gray-700 ${loading ? 'opacity-60 cursor-not-allowed' : 'hover:bg-gray-100'}`}
            >
              취소
            </button>
            <button
