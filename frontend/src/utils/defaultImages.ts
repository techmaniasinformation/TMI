...
// 새로운 이미지 에러 처리 함수들
export const handleImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>, fallbackType: keyof typeof DEFAULT_IMAGES = 'GENERAL') => {
  const img = event.currentTarget;
  img.src = DEFAULT_IMAGES[fallbackType];
  img.onerror = null; // 무한 루프 방지
};

export const handleProfileImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>) => {
  handleImageError(event, 'PROFILE');
};

export const handleCompanyImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>) => {
  handleImageError(event, 'COMPANY');
};

export const handleThumbnailImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>) => {
  handleImageError(event, 'THUMBNAIL');
};

export const handleBadgeImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>) => {
  handleImageError(event, 'BADGE');
};

// 이미지 로딩 상태 관리를 위한 커스텀 훅용 함수
export const createImageErrorHandler = (fallbackType: keyof typeof DEFAULT_IMAGES = 'GENERAL') => {
