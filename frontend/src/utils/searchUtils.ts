...
// URL 파라미터에서 검색 조건 추출
export const extractSearchConditions = (searchParams: URLSearchParams) => {
...
// 검색 조건이 있는지 확인
export const hasSearchConditions = (keyword: string, techTags: string[], companyTags: string[]) => {
...
// 검색 파라미터 생성
export const createSearchParams = (keyword: string, techTags: string[], companyTags: string[], page: number = 1) => {
...
// 태그 제거 로직
export const removeTagFromParams = (
  searchParams: URLSearchParams,
  tagName: string,
  tagType: 'tech' | 'company',
  appliedFilters: any
) => {
  ...
  const tagIndex = appliedFilters?.[`${tagType}Tags`]?.findIndex((t: string) => t === tagName);
