...
function buildError(res, body, fallback) {
  // 서버가 메시지를 주면 그걸 우선 사용
  const serverMsg = body && (body.message || body.error || body.status || body.detail) || '';
...
// 내가 팔로우한 "회원" 목록
export async function getMemberFollows(followerId, page = 0, size = 9) {
...
// 내가 팔로우한 "회사" 목록
export async function getCompanyFollows(followerId, page = 0, size = 9) {
...
// 회원 팔로우
export async function createMemberFollow(followerId, followeeId) {
  ...
  return body; // { status, data: { memberFollowId } }
}

// 회사 팔로우
export async function createCompanyFollow(followerId, companyId) {
  ...
  return body; // { status, data: { companyFollowId } }
}

/** ===================== 삭제 ===================== **/

// 회원 언팔
export async function deleteMemberFollow(followerId, followeeId) {
...
// 회사 언팔
export async function deleteCompanyFollow(followerId, companyId) {
...
export async function findMemberFollowId(followerId, followeeId, pageSize = 50) {
  const list = await getMemberFollows(followerId, 0, pageSize);
  const hit = list.data.memberFollows.find(m => m.memberId === followeeId);
  return hit ? hit.memberFollowId : null;
}

// 회사 팔로우 ID 찾기
export async function findCompanyFollowId(followerId, companyId, pageSize = 50) {
  const list = await getCompanyFollows(followerId, 0, pageSize);
  const hit = list.data.companyFollows.find(c => c.companyId === companyId);
