 ...
 */
export async function updateMemberProfile(memberId, body) {
  ...
  form.append("req", new Blob([JSON.stringify(reqPayload)], {
    type: "application/json"
  }));
  if (usingFile) {
    form.append("profileImage", body.file); // 서버 파트명: profileImage
  }
  try {
    const res = await fetch(`https://i13a509.p.ssafy.io/api/v1/member/${memberId}`, {
      method: "PATCH",
      body: form,
      // Content-Type 수동 지정 금지 (브라우저가 boundary 붙임)
      credentials: "include"
    });
    if (!res.ok) {
...
export async function deleteMember(memberId) {
  try {
    const res = await fetch(`https://i13a509.p.ssafy.io/api/v1/member/${memberId}/delete`, {
      method: "PATCH",
      credentials: "include"
    });
    if (!res.ok) {
