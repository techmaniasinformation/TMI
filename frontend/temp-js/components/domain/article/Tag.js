...
export default function Tag({
  ...
  const getRemoveButtonStyles = variant => {
    switch (variant) {
      case 'company':
  ...
  return /*#__PURE__*/React.createElement("div", _extends({
    className: cn(baseStyles, variantStyles[variant], removable && 'pr-1', className)
  }, props), /*#__PURE__*/React.createElement("span", {
    className: "mr-1"
  }, tag), removable && /*#__PURE__*/React.createElement("button", {
    className: cn("ml-1 w-4 h-4 rounded-full flex items-center justify-center transition-colors", getRemoveButtonStyles(variant)),
    type: "button",
    onClick: e => {
      e.stopPropagation();
      onRemove?.();
