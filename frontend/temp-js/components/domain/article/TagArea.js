...
export default function TagArea({
  ...
  const isTagMatched = tagName => {
    ...
    if (searchTechTags.some(searchTag => lowerTagName === searchTag.toLowerCase())) {
      return true;
    }

    // 회사 태그 검색과 완전히 일치하는지 확인
    if (searchCompanyTags.some(searchCompany => lowerTagName === searchCompany.toLowerCase())) {
      return true;
    }
    return false;
  };
  ...
  }, tags.slice(0, maxTags).map((tag, index) => /*#__PURE__*/React.createElement(Tag, {
    key: index,
    tag: tag,
