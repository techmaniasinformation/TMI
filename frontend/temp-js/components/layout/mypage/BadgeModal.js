...
export default function BadgeModal({
  ...
  const getDefaultMemberBadgeId = () => allMemberBadges.find(mb => mb.badgeId === DEFAULT_BADGE_ID)?.memberBadgeId;
  ...
  const formattedDate = badge.receivedAt ? new Date(badge.receivedAt).toLocaleDateString('ko-KR', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  }) + ' 획득' : '아직 획득하지 않은 배지';
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-light-header dark:bg-dark-header rounded-lg p-6 max-w-md w-full mx-4"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, badge.name), /*#__PURE__*/React.createElement("p", {
    className: "text-gray-600 mb-2 dark:text-gray-300"
    ...
    className: `flex-1 py-2 px-4 rounded-lg transition-colors
                ${submitting || !isOwned && !isRep ? 'bg-gray-300 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed' : isRep ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-blue-600 text-white hover:bg-blue-700'}`
  }, submitting ? '변경 중…' : isRep ? '대표 배지 해제' : '대표 배지로 설정'), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
  ...
  }, "\uCDE8\uC18C")), !isOwned && !isRep && /*#__PURE__*/React.createElement("p", {
