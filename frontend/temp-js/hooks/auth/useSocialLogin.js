...
const useSocialLogin = () => {
  ...
  const handleSocialLoginWithLocation = (provider, from = '/') => {
    ...
    if (isNew === 'false') {
      if (memberId !== null) {
        ...
        fetch(`https://i13a509.p.ssafy.io/api/v1/memberFollow?followerId=${numericMemberId}&all=true`, {
          ...
          const followUserIds = response.data.memberFollows.map(item => item.memberId);
        ...
        fetch(`https://i13a509.p.ssafy.io/api/v1/companyFollow?followerId=${numericMemberId}&all=true`, {
          ...
          const followCompanyIds = response.data.companyFollows.map(item => item.companyId);
