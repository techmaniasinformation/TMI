...
export const useImageCompression = () => {
   ...
   * 내부 유틸: 앞 N바이트 읽기
   */
  const readBytes = (file, n) => new Promise((resolve, reject) => {
   ...
   */
  const detectActualType = async file => {
   ...
   */
  const canDecodeImage = async file => {
    try {
  ...
  const handleImageUpload = async event => {
    ...
    if (!ALLOWED_TYPES.includes(file.type) || !extMime || extMime !== file.type) {
      alert(`jpg, jpeg, png, gif 형식만 업로드할 수 있습니다.\n\n파일명: ${file.name}\n확장자: .${ext}\n감지된 형식: ${file.type || '(알 수 없음)'}`);
      fileInput.value = '';
      return;
    }
    ...
    if (!actual || actual !== file.type) {
      alert(`파일 헤더가 실제 이미지 형식과 일치하지 않습니다.\n\n파일명: ${file.name}\n확장자/MIME: ${extMime}\n실제감지: ${actual ?? '미확인'}\n\n올바른 이미지 파일을 업로드해주세요.`);
      fileInput.value = '';
      return;
    }
    ...
    if (file.size < POST_MIN_SIZE) {
      alert(`파일이 너무 작습니다. 유효한 이미지 파일이 아닐 수 있습니다.\n\n파일명: ${file.name}\n파일 크기: ${file.size} bytes`);
      fileInput.value = '';
      return;
    }
    ...
    try {
      ...
      const sizeOk = await new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
