...
const NICK_COOLDOWN_KEY = memberId => `nicknameCooldown:${memberId}`;
function getDaysLeft(ts) {
  if (!ts) return 0;
...
// 팔로우 API
import { createMemberFollow, deleteMemberFollow, createCompanyFollow, deleteCompanyFollow, findMemberFollowId, findCompanyFollowId } from '@/api/followService';
...
const MyPage = ({
          ...
          try {
            ...
            if (raw) {
              ...
              if ((saved.nickname ?? '') !== (me?.nickname ?? '')) {
                localStorage.setItem(NICK_COOLDOWN_KEY(myId), JSON.stringify({
  ...
  const handleRepChange = useCallback(p => {
    setRepBadge(prev => prev.id === p.badgeId && prev.url === p.badgeUrl ? prev : {
      id: p.badgeId,
      url: p.badgeUrl
    });
  }, []);
  ...
  // 실제 저장 호출로 변경
  const handleProfileSave = async (newNickname, newBlogUrl, newGithubUrl, newProfileUrl, file) => {
    ...
    try {
      ...
      if (file instanceof File) {
        ...
        const willChangeUrl = typeof newProfileUrl !== 'undefined' && newProfileUrl !== (modalInit.profileUrl || '');
        if (willChangeUrl) {
  ...
  const [memberFollowId, setMemberFollowId] = useState(null);
  const [companyFollowId, setCompanyFollowId] = useState(null);
  ...
  }, [isCompany, isMyPage, isFollowing, targetMemberId, targetCompanyId, currentUserId, followUser,
  // ✅ 의존성 추가
  followCompany,
  // ✅ 의존성 추가
  setFollowUser,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getCompany(routeId).then(res => setCompany(res.data)).catch(err => console.error(err)).finally(() => setLoading(false));
  }, [isCompany, routeId]);
  const lastUpdateText = isCompany && company?.lastUpdatedAt ? company.lastUpdatedAt.slice(0, 10) : ''; // ✅ 포맷

  return /*#__PURE__*/React.createElement("div", {
    className: "max-w-[1232px] mx-auto px-4 py-8 bg-light-bg dark:bg-dark-bg"
  ...
  }), /*#__PURE__*/React.createElement(MyPageTabs, {
    isCompany: isCompany,
    isMyPage: isMyPage,
    activeTab: activeTab,
  ...
  }), isMyPage && /*#__PURE__*/React.createElement(ProfileEditModal, {
    ...
    nicknameHelperText: nicknameDaysLeft > 0 ? `닉네임은 ${nicknameDaysLeft}일 후 변경 가능` : undefined,
