...
import { fetchNotifications, deleteNotification as apiDeleteNotification, deleteAllNotifications as apiDeleteAllNotifications, markNotificationRead as apiMarkNotificationRead, markAllNotificationsRead as apiMarkAllNotificationsRead } from '@/api/notification';
...
const BADGE_CDN_BASE = import.meta.env?.VITE_BADGE_CDN ?? `${BASE_URL}/badge/images`;
const cleanUrl = u => u && u.trim() ? u : undefined;
const resolveBadgeSrc = file => {
  const f = (file ?? '').trim();
...
const NotificationsPage = () => {
  ...
  const adapt = useCallback(srv => {
    ...
    let avatar;
    let avatarKind = 'PROFILE';
    if (mappedType === 'badge') {
      const local = badgeImages[srv.badgeUrl];
    ...
    return {
      ...
      badgeType: mappedType === 'badge' ? badgeImages[srv.badgeUrl] ?? resolveBadgeSrc(srv.badgeUrl) : undefined,
      userAvatar: avatar,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      adapted.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  ...
  const handleNotificationClick = async notification => {
    ...
    if (notification.type === 'badge' && memberId) {
      const path = ROUTES?.MY_PAGE ? ROUTES.MY_PAGE.replace(':id', String(memberId)) : `/mypage/${memberId}`;
      navigate(path);
    }
  };
  ...
  if (loading) return /*#__PURE__*/React.createElement(NotificationLoader, null);
  const filteredNotifications = notifications;
  return /*#__PURE__*/React.createElement("div", {
    className: "max-w-[848px] mx-auto bg-light-bg dark:bg-dark-bg"
  }, /*#__PURE__*/React.createElement(NotificationToolbar, {
    showUnreadOnly: showUnreadOnly,
    onToggleUnreadOnly: setShowUnreadOnly,
    onMarkAllAsRead: handleMarkAllAsRead,
    onDeleteAll: handleDeleteAll
  }), loadError && /*#__PURE__*/React.createElement("div", {
    className: "mb-4 p-3 rounded bg-red-50 dark:bg-red-900 text-red-600 dark:text-red-300 text-sm"
  }, loadError), filteredNotifications.length === 0 ? /*#__PURE__*/React.createElement(NoNotifications, {
    showUnreadOnly: showUnreadOnly
  }) : /*#__PURE__*/React.createElement(ClientPagination, {
    data: filteredNotifications,
    currentPage: currentPage,
    pageSize: itemsPerPage,
    onPageChange: setCurrentPage,
    renderItem: notification => /*#__PURE__*/React.createElement(NotificationItem, {
      key: notification.id
